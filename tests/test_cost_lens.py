"""Regression tests for oah.design.lens's cost wiring — same mocked-
...
from oah.design.lens import design_cost, LensDesignError
...
VALID_FRAGMENT = {
    "schema_version": "0.1.0", "lens": "cost", "repo_git_sha": "deadbeef",
    "failure_mode": "fail_open",
    "signals": [
        {
            "name": "oah.cost.attributed_usd", "surface_point_ids": ["sp-0001"],
            "maps_to": {"kind": "oah_extension", "attribute": "oah.cost.attributed_usd"},
            "sensitivity_tier": "internal", "pii_masked": False,
            "supports_decision": "per-workflow spend rollup", "acting_role": "cost owner",
            "latency_overhead_budget_ms": 1,
        },
        {
            "name": "oah.cost.spend_threshold_usd", "surface_point_ids": ["sp-0001"],
            "maps_to": {"kind": "oah_extension", "attribute": "oah.cost.spend_threshold_usd"},
            "sensitivity_tier": "internal", "pii_masked": False,
            "supports_decision": "when to throttle the workflow", "acting_role": "cost owner",
        },
    ],
    "decision_menu_steps": [
        {"step": "throttle generation calls", "type": "throttle",
         "resumption_condition": "oah.cost.rate_limit_headroom > 20%"},
    ],
}
...
def test_no_points_never_calls_the_model():
    calls = []
    result = design_cost([], "deadbeef", _completion_fn=lambda **kw: calls.append(kw))
...
def test_filters_to_llm_generation_kind_only():
    ...
    design_cost([POINT, other_kind], "deadbeef", _completion_fn=fake)
...
def test_prompt_uses_real_skill_instructions_not_a_copy():
    ...
    design_cost([POINT], "deadbeef", _completion_fn=fake)
    system_msg = captured["messages"][0]["content"]
    assert "You design the cost lens's slice" in system_msg


def test_valid_response_returned_and_satisfies_s5_gates():
    result = design_cost([POINT], "deadbeef", _completion_fn=lambda **kw: _fake_response(VALID_FRAGMENT))
...
def test_otel_genai_kind_rejected_this_lens_has_no_upstream_attributes():
    """Unlike generation-capture, cost has no gen_ai.* signals of its own --
...
def test_context_passed_through_when_given():
    ...
    design_cost([POINT], "deadbeef", context=context, _completion_fn=fake)
