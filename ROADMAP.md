...
| **M1 — Auditor** | `oah map` produces a surface map + gap report on real repos | TCR-relevant call-site recall ≥ 90% on reference corpus (Python) | **In progress**, started 2026-08-25. Real `oah` CLI exists (`doctor`/`estimate`/`map`/`inventory`/`gaps`), tree-sitter-based S1/S2/S3(deterministic half), state DB + checkpoint/resume, run_manifest.json. S1 recall reconfirmed at 17/17 (100%) on SP1's pinned corpus — the gate's own metric, not yet re-measured on a larger corpus. Not yet done: S1's LLM disambiguation pass isn't wired to a live model call (candidates route to a batch file, not resolved automatically); S3's owner interview / context.yaml (genuinely interactive, not stubbed); E7's real reference corpus (still SP1's ad hoc 3-repo sample). 37 tests passing. |
