...
| **M2 — Architect** | `oah design` emits architecture, event schema, rollout plan, DTOs | Two pilot products accept an S9 gate report with ≤ minor edits | **Started** 2026-08-25. Full S4→S9 vertical slice now real and wired end to end (`oah design`, `oah event-schema`, `oah dtos`, `oah readiness`), each stage explicitly partial in scope rather than silently presenting partial output as finished: S1 now detects three surface_map point kinds — `oah/discovery/registry.py` generalized from one hardcoded (constructor/module/suffix/kind/framework) tuple into a list of registry entries: pinecone's classic pre-v3 shape (`pinecone.Index("name")` + `.query(...)`, deliberately not the v3+ multi-hop `Pinecone()` client-object shape the detector can't resolve yet, stated not glossed over) → `retrieval`, and langsmith (`Client()` + `.create_feedback(...)`, LangSmith's own real method for binding feedback/reviewer verdicts to a trace — architecture.md's feedback-lens description verbatim) → `feedback_ingest`, both verified against the real vendored corpus not to regress the original llm_generation recall; S4 has 6 of 9 listed lenses built (generation-capture, grounded in SP6's verified `gen_ai.*` attributes; pii-governance, masking/role-access/retention/deletion-by-subject; cost, per-call attribution/spend-thresholds/rate-limit-headroom/throttling; ops, release-id stamping/degradation-visibility/rollback/disablement, with an explicit stated scope line for the two ops sub-items — persistent smoke test, alert plan — that aren't expressible as a design_fragment signal at all; retrieval and feedback, the first two lenses targeting a kind other than `llm_generation` — wiring retrieval caught a real bug where every S5-gate-running command checked every fragment against one hardcoded llm_generation-only point-id list regardless of which kind that fragment's own lens targets, fixed via a `LENS_TO_POINT_KIND` lookup (confirmed by reverting the fix and watching the regression test fail exactly as predicted before restoring it) and proven to actually generalize — not just work for a hardcoded pair — when feedback landed cleanly against the same mechanism with a 3-kind CLI test; pii-governance/cost/ops/retrieval/feedback all design `oah.*` extensions only, since none has an upstream semconv attribute, and cost was the first lens to exercise `decision_menu_steps`; all six LiteLLM-wired through the same generalized `design_lens()` wiring); every S4-running command (`oah design`, `event-schema`, `dtos`, `readiness`) treats a lens failure as a warning and proceeds with whichever lenses did produce a fragment, not a hard failure; S5's deterministic gates are fully built (pure code, all of architecture.md's checks, one stated scope boundary around judging *which* fields could contradict — referential integrity of declared assertions is checked, "needed one and didn't have it" is advisory not authoritative); S6 now has all 3 of 3 personas built (cost_skeptic; sre — cardinality, combined per-point overhead, unbounded-volume signals, with an explicit note that "collector failure modes" can't fail by construction since failure_mode is a schema const and is correctly excluded rather than faked as a checkable finding; security — prompt/output sensitivity-tier mismatches S5 doesn't judge, the telemetry path's own injection surface, plus two context.yaml-grounded checks, with "staging-as-production-evidence" and "tool-boundary" explicitly out of scope until S10/S11 and a tools lens exist); S7's event-schema merge is fully built (pure code, cross-fragment attribute-conflict detection); S8's DTO generation is LLM-driven with deterministic `rollout_step` assignment now following architecture.md S7's actual stated rule — `(workflow_criticality, workflow_name, dimension_tier, gap_priority, dto_id)` from `--context`, gracefully falling back to gap-priority-only ordering when no context.yaml is given — closing the "stand-in" honestly flagged since S8 first landed; caught two prerequisite bugs while wiring it: retrieval-kind points silently produced no gap at all (`gap_model.py`'s `KIND_TO_DIMENSION` only ever mapped `llm_generation`), and `oah dtos` had no `--context` flag at all so this ordering rule had no way to activate from the CLI regardless of the underlying logic — both fixed; S9's readiness report is deterministic assembly only (architecture.md marks it as such) — it never returns "ready" outright at this pipeline stage since no S10 instrumentation or S11 validation evidence exists yet, capping at "ready_with_conditions", and states its own scope gaps in `known_limitations`/`evidence_position.unknown` rather than glossing over them. M2's own remaining scope is now just 3 more S4 lenses (tools, realtime-multimodal — each still needs S1 to detect its own new surface-point kind; tracing — cross-cutting, explicitly the hardest lens, not point-scoped at all); S10/S11 belong to M3/M4 respectively, not M2. 213 tests passing. |
