...
expected telemetry event is actually emitted at runtime), which remains
blocked on `skills/s10-instrumenter/SKILL.md` never naming a concrete
telemetry API for the agent to call — the same gap M4's status note
already flagged, still not fixed, named again here rather than quietly
worked around. What `--dynamic` does add is real: `verdict` grew a
