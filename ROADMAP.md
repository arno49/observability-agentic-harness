...
| **M2 — Architect** | `oah design` emits architecture, event schema, rollout plan, DTOs | Two pilot products accept an S9 gate report with ≤ minor edits | **Started** 2026-08-25. Full S4→S9 vertical slice real and wired end to end (`oah design`, `event-schema`, `dtos`, `readiness`), every stage explicitly partial rather than silently presenting partial output as finished. **S1**: `oah/discovery/registry.py` generalized to a list of registry entries and now detects 4 surface-point kinds — `llm_generation` (anthropic, E2's original target), `retrieval` (pinecone), `feedback_ingest` (langsmith), `realtime_session` (livekit — first registry needing a new import-tracking shape, `from pkg import submodule`, added to `ImportResolver` for it) — each entry's real-world-fit tradeoffs (suffix genericity, which SDK generation/import shape is/isn't supported) stated in the module docstring, not glossed over; also now determines `sync_nature` (async/sync) deterministically from tree-sitter's own `async` token, grounding S4's tracing lens in a real AST fact rather than a guess; corpus recall unregressed at each step. **S4**: 8 of 9 lenses built — generation-capture (SP6-verified `gen_ai.*` attributes), pii-governance, cost, ops, retrieval, feedback, realtime-multimodal (all `oah.*` extensions only, no upstream semconv attribute for any of them), and tracing — architecture.md's own "hardest lens," grounded directly in SP2's decision record (`docs/trace-propagation-patterns.md`) which already assigns most of tracing's real complexity to S1/S2 as presence-detection work, not S4 design; the lens itself is deliberately narrow (one signal: same-process-asyncio-verified-safe vs. everything-else-unverified) and is the first cross-cutting lens — not scoped to one point kind, wired via `LENS_TO_POINT_KIND["tracing"] = None` meaning "every point," verified with both a positive and a negative gate-check control; wiring the first non-`llm_generation` lens (retrieval) caught a real cross-kind gate-checking bug in the CLI (fixed via `LENS_TO_POINT_KIND`, confirmed by reverting and watching the regression test fail as predicted), later proven to actually generalize via 4-kind and cross-cutting CLI tests. **S5** gates fully built (pure code; one stated scope boundary on judging *which* fields could contradict). **S6**: all 3 personas built (cost_skeptic, sre, security) — sre explicitly excludes a "collector failure modes" check since that can't fail by schema construction, security covers what S5 doesn't judge (is a tier *right*, not just internally consistent) plus context.yaml-grounded trust-boundary/governance checks. **S7** event-schema merge fully built (pure code, cross-fragment conflict detection). **S8** DTO generation is LLM-driven with rollout_step now following architecture.md's real workflow-criticality ordering rule from `--context` (closing the gap-priority-only stand-in), which itself required fixing two prerequisite gaps (`gap_model.py` silently dropping non-generation-capture points; `oah dtos` missing `--context` entirely). **S9** readiness report is deterministic assembly, capped at `ready_with_conditions` (no S10/S11 evidence exists at this stage), states its own scope gaps rather than glossing over them. M2's own remaining scope: just the tools lens now (no clean SDK-signature detection story — a tool call isn't a distinct SDK call site, it's an application-defined handler invoked in response to a `tool_use` block, needing a genuinely different detection heuristic); S10/S11 belong to M3/M4, not M2. 244 tests passing. |
