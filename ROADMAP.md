...
| **M1 — Auditor** | `oah map` produces a surface map + gap report on real repos | TCR-relevant call-site recall ≥ 90% on reference corpus (Python) | **In progress**, started 2026-08-25. Real `oah` CLI (`doctor`/`estimate`/`map`/`inventory`/`gaps`/`interview`): tree-sitter S1 (LLM disambiguation wired to a real LiteLLM call, SP8's frontier default, checkpointed independently from the deterministic scan), S2, S3 (deterministic join + a real interactive owner-interview producing `context.yaml`, wired into gap priority weighting), state DB + checkpoint/resume, run_manifest.json. S1 recall reconfirmed at 17/17 (100%) on SP1's pinned corpus — the gate's own metric, not yet re-measured on a larger corpus. Not yet done: E7's real reference corpus (still SP1's ad hoc 3-repo sample) — the only concretely tracked gap left before this gate can be claimed with confidence rather than a thin sample. 59 tests passing. |
