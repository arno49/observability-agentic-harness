...
| **M2 — Architect** | `oah design` emits architecture, event schema, rollout plan, DTOs | Two pilot products accept an S9 gate report with ≤ minor edits | **Started** 2026-08-25. S5's deterministic gates fully built (pure code, all of architecture.md's checks, one stated scope boundary around judging *which* fields could contradict — referential integrity of declared assertions is checked, "needed one and didn't have it" is advisory not authoritative). S4: 1 of 9 listed lenses built for real (generation-capture, LiteLLM-wired, grounded in SP6's verified `gen_ai.*` attributes). `oah design` runs what exists and says explicitly it's partial, not silently producing an incomplete design as if finished. Remaining: 8 more lenses, S6's adversarial panel, S7-S9 synthesis. 91 tests passing. |
